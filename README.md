...
## الزخارف الشهرية (الخلفية المائية للطباعة)

كل شهر له رسمة خلفية مائية اختيارية تظهر خلف رزنامة الطباعة (بشفافية منخفضة كـ watermark). الرسمات ملفات SVG مستقلة في:

```
public/monthly-art/01.svg … 12.svg   (01 = يناير … 12 = ديسمبر)
```

الرسمات الحالية مبدئية (أسلوب خطّي كويتي: مركب، أبراج الكويت، سدو، نخلة، بوابة السور، مسجد، دلة، لؤلؤ ومحار، صقر، برج التحرير، العلم، الأرفج).

**لاستبدال أي رسمة برسمتك الخاصة:** استبدل الملف بنفس الاسم (مثل `07.svg` لشهر يوليو) — بدون أي تعديل على الكود. توصيات:

- استخدم **SVG** بـ `viewBox="0 0 240 240"` (مربّع) ليتوسّط بشكل صحيح.
- التطبيق يعرض الرسمة بشفافية منخفضة تلقائيًا (`opacity ≈ 0.05`)، فلا حاجة لتفتيحها بنفسك؛ صمّمها بألوانها الطبيعية.
- اجعلها بسيطة وواضحة حتى تبقى الأرقام مقروءة فوقها.
- يمكن استخدام PNG أيضًا (نفس الاسم `07.png`)؛ لكن عدّل الامتداد في `components/PrintMonth.tsx` (مصدر الصورة `monthly-art/${nn}.svg`).

خيار «زخرفة الشهر» في شاشة الطباعة يشغّل/يطفئ هذه الخلفية.
